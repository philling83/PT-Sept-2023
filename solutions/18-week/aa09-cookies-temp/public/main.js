...
function storeTheme(themeName) {
    const newCookie = `themeName=${themeName};max-age=5`;
...
function restoreTheme() {
    const cookieObj = spliceCookie();
    // console.log(cookieObj);
    if (cookieObj['themeName']) {
        setTheme(cookieObj['themeName']);
    }
...
function restoreName() {
    ...
    if (cookieObj['displayName']) {
        setInputValue('display-name', cookieObj['displayName']);
    }
}
